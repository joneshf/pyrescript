...
import qualified Language.PyreScript.Make as Make
...
compile PSCMakeOptions{..} = do
  ...
  (makeErrors, makeWarnings) <- Make.runMake pscmOpts $ do
    ...
    let makeActions = Make.buildMakeActions pscmOutputDir filePathMap foreigns pscmUsePrefix
...
globWarningOnMisses warn = concatMapM globWithWarning
  where
  globWithWarning pat = do
    paths <- glob pat
    when (null paths) $ warn pat
    return paths
